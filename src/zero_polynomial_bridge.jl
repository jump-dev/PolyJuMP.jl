...
function ZeroPolynomialBridge{T, F}(model::MOI.ModelLike,
                                    ...
                                    s::MOI.ZeroPolynomialSet{<:MOI.MonomialBasis}) where {T, F}
...
function MOI.supports_constraint(::Type{ZeroPolynomialBridge{T}},
                                 ...
                                 ::Type{<:MOI.ZeroPolynomialSet}) where T
    return true
end
function added_constraint_types(::Type{<:ZeroPolynomialBridge{T, F}}) where {T, F}
    return [(F, MOI.Zeros)]
end
function concrete_bridge_type(::Type{<:ZeroPolynomialBridge},
                              F::Type{<:MOI.AbstractVectorFunction},
                              ::Type{MOI.ZeroPolynomialSet{<:MonomialBasis, MT, MVT}}) where {T, MT, MVT}
...
function MOI.get(model::MOI.ModelLike, a::MOI.ConstraintDual,
                 c::ZeroPolynomialBridge)
    return MOI.get(model, MOI.ConstraintDual(), c.zero_constraint)
end
