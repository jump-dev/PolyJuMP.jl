...
function test_NonNeg(var)
    ...
    _test_constraint(m, @constraint(m, p <= q),
            S, jump_set, -p + q, [], [])
    _test_constraint(m, @constraint(m, q - p in PolyJuMP.NonNegPoly()),
            S, jump_set, q - p, [], [])
    _test_constraint(m, @constraint(m, p + q >= 0, domain = @set x == y^3),
            S, jump_set, p + q, [], [x - y^3])
    @testset "Custom keyword" begin
        _test_constraint(m, @constraint(m, p <= q, maxdegree=1),
...
function test_ZeroPolynomialSet(var)
    ...
    _test_constraint(m, @constraint(m, p == q, domain = dom),
...
function test_PosDefMatrix(var)
    ...
    _test_constraint(m, @constraint(m, [p q; q 0] - [0 0; 0 p] in PSDCone()),
            DummyPolyModule.PosDefMatrix, jump_set, [p q; q -p], [], [])
    _test_constraint(m, @constraint(m, [p q; q 0] >= [0 0; 0 p], PSDCone()),
            DummyPolyModule.PosDefMatrix, jump_set, [p q; q -p], [], [])
    _test_constraint(m, @constraint(m, [0 0; 0 p] <= [p q; q 0], PSDCone()),
            DummyPolyModule.PosDefMatrix, jump_set, [p q; q -p], [], [])
end
