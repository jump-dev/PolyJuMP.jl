@testset "@variable macro with Poly" begin
    ...
    function testvar(m, p, x, binary=false, integer=false, vars=true)
        @test isa(p, vars ? DynamicPolynomials.Polynomial{true,JuMP.VariableRef} : DynamicPolynomials.Polynomial{true,JuMP.AffExpr})
        @test p.x == x
        if vars
    ...
    @testset "MonomialBasis" begin
        ...
        @test isa(p1, Vector{DynamicPolynomials.Polynomial{true,JuMP.VariableRef}})
        testvar(m, p1[1], X)
        @variable(m, p2, Poly(X), integer=true)
        testvar(m, p2, X, false, true)
        @variable(m, p3[2:3], Poly(X))
        @test isa(p3, JuMP.JuMPArray{DynamicPolynomials.Polynomial{true,JuMP.VariableRef},1,Tuple{UnitRange{Int}}})
        testvar(m, p3[2], X)
    ...
    @testset "FixedPolynomialBasis" begin
        ...
        testvar(m, p4[1], monovec([x^2, x, 1]), false, true, false)
...
@testset "resultvalue function" begin
    m = Model()
    @variable m α
    @variable m β
    @polyvar x y
    ...
    JuMP.fix(α, 2)
    JuMP.fix(β, 3)
    @test_broken JuMP.resultvalue(p) == 2x*y + 3x^2
end
